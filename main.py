...
def crear_movimiento(user_id: int, mvm: schemas.MovimientoRegister, db: Session = Depends(get_db)):
    db_user = op_monetarias.obtener_cliente_por_id(db, user_id)
    if not db_user:
        raise HTTPException(status_code=400, detail="este id de cliente no existe")
    return op_monetarias.crear_movimiento_a_cliente(db=db, id_cliente=user_id, mvm=mvm)
