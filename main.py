...
async def crear_cuenta(user_id: int, cta: schemas.CuentaRegister, db: Session = Depends(get_db)):
    db_user = op_monetarias.obtener_cliente_por_id(db, user_id)
    if not db_user:
...
@app.post("/clientes/{id_cuenta}/movimientos")
